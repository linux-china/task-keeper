...
pub fn get_task_command_map() -> HashMap<String, String> {
    ...
    if let Ok(code) =std::fs::read_to_string("gradle/wrapper/gradle-wrapper.properties") {
        if !code.contains("gradle-8.6") {
            task_command_map.insert("self-update".to_string(), format!("{} wrapper --gradle-version=8.6", gradle_command));
        }
    }
    task_command_map
}
...
fn get_start_command_line() -> String {
    ...
    if std::env::current_dir().map(|dir| dir.join(build_gradle_file).exists()).unwrap_or(false) {
        ...
        return if (build_gradle_file == "build.gradle.kt" && gradle_build_code.contains(r#"id("org.springframework.boot")"#))
            ...
            format!("{} --console=plain quarkusDev", get_gradle_command())
        } else {
            format!("{} run", get_gradle_command())
        };
    } else {
        format!("{} run", get_gradle_command())
    }
